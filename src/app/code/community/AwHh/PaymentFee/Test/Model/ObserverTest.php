<?php
class AwHh_PaymentFee_Test_Model_ObserverTest extends EcomDev_PHPUnit_Test_Case
{
    /**
    ...
    public function testObserverModuleDisabled()
    {
        ...
        $fees_enabled = Mage::getConfig()->setNode('payment_services/paymentfee/payment_methods');
	...
	/**
	 * observer must add payment fee if customer selected a
	 * payment method that was enabled for payment fee via backend.
	 * - module is enabled
	 * - current payment method is 'checkmo'
	 * - payment method 'checkmo' is enabled for payment fee in backend config
	 */
    public function testObserverPaymentMethodSelectedApplicable()
    {
        // set config as needed in current test
        $active = Mage::getStoreConfig('payment_services/paymentfee/active');
        if (!$active) {
        ...
        // test if payment fee equals 0
        $this->assertEquals(10, $event->getQuote()->getPaymentFee());
